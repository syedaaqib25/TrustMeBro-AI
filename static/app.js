...
function showResults(data) {
    ...
    if (gemini.score !== null && gemini.score !== undefined) {
        const gs = Math.round(gemini.score);
        renderScoreCard(geminiCardId, gs);
        const reasonEl = document.getElementById('gemini-reasoning');
        if (gemini.reasoning) {
            reasonEl.textContent = gemini.reasoning;
            reasonEl.classList.remove('hidden');
        } else {
            reasonEl.classList.add('hidden');
        }
    } else {
        // Gemini failed or missing
        document.getElementById(`value-${geminiCardId}`).textContent = 'N/A';
        const ring = document.getElementById(`ring-${geminiCardId}`);
        if (ring) ring.style.strokeDashoffset = '326.7';
        const badge = document.getElementById(`badge-${geminiCardId}`);
        if (badge) {
            badge.textContent = gemini.error ? 'Error' : 'N/A';
            badge.className = 'score-badge uncertain';
        }
        const reasonEl = document.getElementById('gemini-reasoning');
        if (reasonEl) {
            reasonEl.textContent = gemini.error || 'Gemini unavailable';
            reasonEl.classList.remove('hidden');
        }
    }

    // Render Groq
    const groqCardId = 'groq';
    const groq = data.groq || {};
    if (groq.score !== null && groq.score !== undefined) {
        const gs = Math.round(groq.score);
        renderScoreCard(groqCardId, gs);
        const groqReasonEl = document.getElementById('groq-reasoning');
        if (groq.reasoning) {
            groqReasonEl.textContent = groq.reasoning;
            groqReasonEl.classList.remove('hidden');
        } else {
            groqReasonEl.classList.add('hidden');
        }
    } else {
        document.getElementById(`value-${groqCardId}`).textContent = 'N/A';
        const ring = document.getElementById(`ring-${groqCardId}`);
        if (ring) ring.style.strokeDashoffset = '326.7';
        const badge = document.getElementById(`badge-${groqCardId}`);
        if (badge) {
            badge.textContent = groq.error ? 'Error' : 'N/A';
            badge.className = 'score-badge uncertain';
        }
        const groqReasonEl = document.getElementById('groq-reasoning');
        if (groqReasonEl) {
            groqReasonEl.textContent = groq.error || 'Groq unavailable';
            groqReasonEl.classList.remove('hidden');
        }
    }
...
async function submitFeedback() {
    ...
    try {
        const payload = {
            ...
            groq_score: lastAnalysis.groq?.score,
...
async function loadFeedback() {
    ...
    try {
        ...
        if (!items.length) { tbody.innerHTML = '<tr><td colspan="10" class="table-empty">No feedback yet.</td></tr>'; return; }
        ...
        tbody.innerHTML = items.map(item => {
            ...
            const groqVal = item.groq_score != null ? Math.round(item.groq_score) : '—';
            const groqHtml = typeof groqVal === 'number' ? `<span class="score-${groqVal >= 70 ? 'green' : groqVal >= 40 ? 'yellow' : 'red'}">${groqVal}%</span>` : '—';
...
async function exportFeedbackCSV() {
    try {
        ...
        const headers = ['Timestamp', 'Verdict', 'Explanation', 'LR', 'SVM', 'NB', 'LGBM', 'Gemini', 'Groq', 'Snippet'];
        ...
        items.forEach(item => {
            ...
            const row = [
                ...
                item.groq_score != null ? Math.round(item.groq_score) : '',
...
async function loadHeadlines(category = _currentCategory) {
    ...
    try {
        const res = await fetch(`${API}/newsfeed?category=${category}`);
        if (!res.ok) {
            const err = await res.json().catch(() => ({}));
